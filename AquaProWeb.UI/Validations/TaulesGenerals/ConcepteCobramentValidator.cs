...
{
	public class ConcepteCobramentValidator : AbstractValidator<CreateConcepteCobramentDTO>
	{
		...
		{
			var result = await ValidateAsync(ValidationContext<CreateConcepteCobramentDTO>.CreateWithOptions((CreateConcepteCobramentDTO)model, x => x.IncludeProperties(propertyName)));
			if (result.IsValid)
