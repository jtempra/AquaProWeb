...
{
	public class ConcepteFacturaValidator : AbstractValidator<CreateConcepteFacturaDTO>
	{
		...
		{
			var result = await ValidateAsync(ValidationContext<CreateConcepteFacturaDTO>.CreateWithOptions((CreateConcepteFacturaDTO)model, x => x.IncludeProperties(propertyName)));
			if (result.IsValid)
